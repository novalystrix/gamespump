  ...
  try {
    ...
    const [totals, pageViews, gameStarts, gameEnds, byDay] = await Promise.all([
      ...
      query<{ game_type: string; cnt: string; avg_players: string }>(`
        SELECT
          COALESCE(props->>'gameType', props->>'gameId') AS game_type,
          COUNT(*) AS cnt,
          AVG((props->>'playerCount')::numeric) AS avg_players
        FROM analytics_events
        WHERE event = 'game_start' AND created_at >= $1 AND (props->>'gameType' IS NOT NULL OR props->>'gameId' IS NOT NULL)
        GROUP BY game_type
        ORDER BY cnt DESC
      `, [since]),

      query<{ game_type: string; cnt: string }>(`
        SELECT COALESCE(props->>'gameType', props->>'gameId') AS game_type, COUNT(*) AS cnt
        FROM analytics_events
        WHERE event = 'game_end' AND created_at >= $1
          AND (props->>'gameType' IS NOT NULL OR props->>'gameId' IS NOT NULL)
        GROUP BY game_type
      `, [since]),
