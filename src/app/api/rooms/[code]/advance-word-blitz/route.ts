...
import { advanceWordBlitzRound, forceWordBlitzResults } from '@/lib/rooms';
...
export async function POST(
  ...
  try {
    ...
    if (action === 'force-results') {
      const room = forceWordBlitzResults(params.code);
      if (!room) {
    ...
    const room = advanceWordBlitzRound(params.code);
    if (!room) {
